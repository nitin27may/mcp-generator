...
| P2-W23-E01 | **In progress (7/11 routes).** Next.js Route Handlers per §53 (not Fastify — §93 C21); job pattern deferred, not yet needed. Done: `POST /api/import`, `POST /api/projects`, `GET /api/projects/:id`, `POST /api/projects/:id/analyze`, `PUT /api/projects/:id/config`, `POST /api/projects/:id/playground/dry-run`, `POST /api/projects/:id/playground/execute` (Increment 10 — see `P4-W20-E02`/`E03`). `packages/control-contracts` and the disk-backed project store landed alongside. Remaining: `generate`, `source-versions`, `diff` | P2 | P1-W26-T01 | L | 6–10 | §53 |
