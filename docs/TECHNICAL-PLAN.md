...
| P2-W23-E01 | **In progress (6/11 routes).** Next.js Route Handlers per §53 (not Fastify — §93 C21); job pattern deferred, not yet needed. Done: `POST /api/import`, `POST /api/projects` (seeds a draft `McpProjectConfig` via `seedProjectConfig`), `GET /api/projects/:id` (with `computeGates`; `?include=` supports `analysis`, `operations`, and `operationDetail`+`operationId`), `POST /api/projects/:id/analyze`, `PUT /api/projects/:id/config`, `POST /api/projects/:id/playground/dry-run` (Increment 9 — see `P4-W20-E01`). `packages/control-contracts` and the disk-backed project store landed alongside; store gained `readProjectAnalysis`/`writeProjectAnalysis`/`updateProjectConfig`. Remaining: `playground/execute`, `generate`, `source-versions`, `diff` | P2 | P1-W26-T01 | L | 6–10 | §53 |
