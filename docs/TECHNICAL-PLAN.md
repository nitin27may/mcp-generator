...
| P2-W19-E04 | **Binding UI done; inheritance display/reset-to-inherited out of scope (P1-W05-T01, config inheritance engine, isn't built — no groups to inherit from).** Increment 7 of the web-UI plan: `GET ...?include=operationDetail&operationId=` (already landed in Increment 6) feeds `/bindings`. `BindingTable`: one row per operation parameter + request-body property, all 4 `ValueBinding` kinds selectable (`ValueBindingField` generalized from Increment 5's 3-kind version — `tool-input` now included, since that's the one kind that actually belongs here), and a real, reachable "unbound" state (a `Remove` button deletes the binding key entirely rather than resetting to a placeholder) — without that, `BND-001` could never actually surface, since every parameter starts pre-seeded with a tool-input binding. `computeBindingDiagnostics` (new, pure, in `control-contracts` — a static presence-only counterpart to `binding-engine`'s runtime `BND-001` check, which needs already-resolved values that don't exist at config-editing time) renders live, client-side, before any server round trip. `EnvVarSummary` (BRD §16's configuration preview) aggregates every `environment`/`secret` binding across `api.baseUrl`, `upstreamAuthentication`, and every *enabled* tool's bindings, via a new pure `buildEnvVarSummary` — deliberately drops the BRD mockup's `type`/`default` columns, which this build can't honestly infer from a bare binding. Verified against the real running server: enabled a tool with a required request-body property, removed its binding via the real `PUT /config` (exactly what the UI's `Remove` button produces), confirmed `gates.bindings.complete` flips to `false` and back to `true` on restore, and confirmed the client-side `computeBindingDiagnostics` logic flags the exact same field the server's gate does | P2 | P2-W19-E03, P1-W05-T01 | L/XL | 8–12 | FR-BIND-001…007, FR-INH-003/004, FR-CFG-001…003 |
...
| P2-W23-E01 | **In progress (9/11 routes).** Next.js Route Handlers per §53 (not Fastify — §93 C21); job pattern deferred, not yet needed. Done: `POST /api/import`, `POST /api/projects`, `GET /api/projects/:id`, `POST /api/projects/:id/analyze`, `PUT /api/projects/:id/config`, `POST /api/projects/:id/playground/dry-run`, `POST /api/projects/:id/playground/execute` (Increment 10 — see `P4-W20-E02`/`E03`), `POST /api/projects/:id/generate`, `GET /api/projects/:id/generate/:buildId/download` (Increment 11 — see `P4-W20-E03`). `packages/control-contracts` and the disk-backed project store landed alongside. Remaining: `source-versions`, `diff` | P2 | P1-W26-T01 | L | 6–10 | §53 |
...
| P3-W19-E01 | **Done — both halves.** Readiness half from Increment 4 (see below). Safety half from Increment 8 of the web-UI plan: `/projects/:id/policy` — a table of every *enabled* tool with `RiskOverrideCell` (a select over all 5 `ToolRisk` values, showing the original `risk-engine` suggestion + its `reasons` alongside as context, not as the editable value — overriding it is the point) and `RetryToggle` (a genuine 3-state control — default/always/never, since `ToolConfig.retry` is fully optional and collapsing that into a boolean would lose the "unset" state; disabled with explanatory copy, not hidden, when the tool's *current* risk is `DESTRUCTIVE`/`PRIVILEGED`, since `upstream-http`'s `isRetryEligible` keys off the tool's live configured risk, not a frozen original classification — changing risk, not the retry control, is what actually lifts BR-006's floor). Two non-dismissible notes cover the scope gaps: no confirmation-before-execution field yet (`MCP-005` reserved, unbuilt), and the retry-floor behavior explained accurately rather than left implicit. Verified against the real running server: overrode a tool to `risk: DESTRUCTIVE` + `retry: {enabled: true}` and confirmed the config round-trips (200, not rejected — the floor is a runtime concern in `upstream-http`, not a config-schema concern) while `packages/upstream-http/src/retry-policy.test.ts`'s own existing, untouched unit test already proves that exact combination is refused by `isRetryEligible` at execution time — the UI's claim and the runtime's behavior were cross-checked, not just asserted in copy | P3 | P3-W16-E02, P2-W19-E05 | M | 4–6 | FR-ARA-002, FR-POL-001/002 |
| P4-W20-E01 | **Done.** Dry-run request preview with unresolved-variable detection — Increment 9 of the web-UI plan, and the first increment verified with a real browser (Playwright), not just curl. `POST /api/projects/:id/playground/dry-run` (`src/server/playground.ts`'s `performDryRun`): calls `resolveBindingValues`+`buildHttpRequestParts` directly (never `buildToolRegistry`, which would attach auth and execute — that's `P4-W20-E02`), `resolveSecret` always returns `undefined`. `AUT-001`/`BND-005` (unresolved secret/env) become placeholder substitutions (`<SECRET:NAME>`/`<ENV:NAME>`) reported as `unresolvedVariables`, not blocking errors — the point is to preview the shape of the request, not require a fully-deployed environment first. `HttpRequestParts.query` (a `URLSearchParams`) is re-shaped to a tuple array for the wire format, since `JSON.stringify` silently drops it otherwise. `ToolInputForm`/`RequestPreview`/`PlaygroundToolPanel` (a component keyed by tool name so switching tools remounts cleanly, rather than an effect calling `setState` on prop change — the latter is a real React anti-pattern the lint config catches). Verified via a real fixture dry-run (`GET /customers/cust_123` with the path placeholder correctly substituted) and an env-override case (base URL resolves when a real value is supplied, placeholder-substituted otherwise) | P4 | P2-W19-E04 | M | 4–6 | FR-DRY-001…003, FR-PLAY-005 |
| P4-W20-E02 | **Done, together with E03 (built as one increment — the egress controls aren't separable from live execution, they're the same code path).** Live playground + redacted trace + destructive acknowledgement — Increment 10 of the web-UI plan. `POST /api/projects/:id/playground/execute` (`src/server/execute.ts`'s `performExecute`) does exactly two things `buildToolRegistry` doesn't already do — a `DESTRUCTIVE`/`PRIVILEGED` risk gate (428 `PLG-001` without `acknowledgeRisk`) and an egress policy — then calls the *real* `tool.execute()`, not a second reimplementation (R5: the playground can't drift from what a generated server actually does). `ExecutionTrace` (TIP §36) is built by wrapping `fetchImpl` in a recording proxy that captures the outgoing request and response status/duration — never by re-deriving the request via a second `buildHttpRequestParts` call, which would let the trace silently disagree with what actually executed. Secrets are request-scoped only (never persisted, held in a `PlaygroundToolPanel` `useState` that's wiped on tool-switch since the component remounts via `key`). `RiskAcknowledgeDialog`/`TracePanel` round out `/playground`. Verified with two dedicated real-browser Playwright specs (not curl): a full live execute against a real local fixture API showing the actual response data and a redacted `Authorization` header with the raw secret literal absent from the entire rendered page, and the risk dialog blocking/cancelling a `Destructive` tool. New `apps/web/test/security/execute-ssrf.test.ts`: blocks a cloud metadata address, an RFC 1918 address, and loopback by default; allows loopback (and gets a real 200 from the fixture API) only with `MCPGEN_ALLOW_PRIVATE_EGRESS=true`; confirms a session-scoped secret never appears in a blocked-egress outcome either. One real bug found and fixed in the same pass: `TracePanel` built `resolvedRequest.headers` server-side but never actually rendered them client-side, so a user had no way to see that a redacted `Authorization` header was even attempted | P4 | P4-W20-E01, P1-W27-T01 | L | 6–10 | FR-PLAY-001…006, §36 |
| P4-W20-E03 | **Done.** Generate + download — Increment 11 of the web-UI plan. `POST /api/projects/:id/generate` (`src/server/generate.ts`'s `performGenerate`) runs `parseProjectConfig` then the real `generateProject()` (`@mcpgen/generator`, the same function `apps/cli`'s `generate` command calls, `P2-W15-E04` — no duplicated logic) into `out/<buildId>/`, walks the output for a file manifest, and enforces a byte cap (`MCPGEN_MAX_BUILD_BYTES`, new `GEN-006`) with cleanup on every failure path. `GET /api/projects/:id/generate/:buildId/download` streams a real zip via `yazl` (`Readable.toWeb(zip.outputStream as Readable)` as the Route Handler body — `ZipFile.outputStream` is typed as the looser `NodeJS.ReadableStream`, needing the cast). Build sweeper (`sweepBuilds`, `MCPGEN_BUILD_TTL_MINUTES`, default 15) added to the existing project-store TTL sweep. `GenerateView`/`FileManifest`/`DownloadButton` (`DownloadButton` is a real `<a download>`, not a blob-URL `createObjectURL` dance — see D4). Turbopack needed `serverExternalPackages: ['esbuild']` in `next.config.ts` (it otherwise tries to trace esbuild's platform-native binary into the route bundle, since `generateProject`'s internal `bundleRuntime` step imports `esbuild`). Verified end to end for real: downloaded zip extracts and `node dist/cli.mjs serve` actually runs; `apps/web/e2e/wizard-happy-path.spec.ts` extended through Generate in a real browser (button click → real manifest with `package.json`/`dist/cli.mjs` → real download event with a `.zip` filename). One significant pre-existing bug found by this real-browser run and fixed in the same pass — see §93 C29 | P4 | P4-W20-E02, P2-W15-E04 | M | 4–6 | FR-GEN-001…005, §53 |
...
| P4-W19-E01 | **Done.** Accessibility + E2E hardening — Increment 12 of the web-UI plan, the last of the increments driving real-browser verification (C26–C30 already fixed skip-link/heading/role, `<Select>` labels, save/loading-state gaps, and the generate gate; this increment adds the systematic axe/keyboard coverage that makes those regressions provable in CI going forward, not just caught by hand). New: `StepShell` moves focus to the step's `<h1>` (`tabIndex={-1}`, focused via a `usePathname()`-keyed `useEffect`, skipped on first mount) on every step transition — App Router doesn't reset focus like a traditional MPA does, so without this a screen-reader user's focus silently stayed on whatever control they'd clicked in the previous step. `apps/web/e2e/a11y.spec.ts`: `@axe-core/playwright` against all 10 wizard routes (a shared `seedReadyProject` fixture creates a project with one enabled, fully-bound tool via the real API so every step renders its real, not blocked-state, content) — asserts zero `serious`/`critical` violations per route; this run found and fixed a genuine WCAG AA contrast failure shared by `Button`'s and `Badge`'s `destructive` variant (`text-destructive` on `bg-destructive/10` was a 4.0:1 ratio against the 4.5:1 minimum — now `text-red-700`/`dark:text-red-400`, same hue, passing contrast), affecting every destructive-styled control in the wizard (Remove-binding buttons, required-field badges, etc.) via the one shared-component fix. `apps/web/e2e/keyboard.spec.ts`: keyboard-only (no `.click()`) traversal of the two custom widgets — `OperationTable`'s roving-tabindex rows (arrow keys, Enter, independently-Tab-reachable checkbox) and `BindingTable`'s kind `<Select>` + Remove button — which found a second real bug: the enable checkbox's `Space` keydown bubbled up into the row's own roving-tabindex handler (which calls `preventDefault()` and opens the tool designer instead), so a keyboard user pressing Space on a focused checkbox couldn't actually toggle it; fixed with the same `stopPropagation()` pattern the checkbox's `onClick` already used, just added to `onKeyDown`. Both findings recorded as §93 C31. `aria-live`/diagnostic announcements (`SaveIndicator`'s `aria-live="polite"`, `ProductErrorPanel`'s `role="alert"`) and Monaco's accessible group label were already in place from earlier increments and are exercised (not re-verified in isolation) by the same specs. New blocking `web-e2e` CI job (`.github/workflows/ci.yml`) runs the full Playwright suite (`playwright install --with-deps chromium` — GitHub-hosted runners have the root access this sandbox's manual verification didn't). **Manual screen-reader pass — not run in this environment (no real AT available here); left as an explicit human checklist**, since heading-navigation/focus-management/dialog-focus-trap *behavior* is what axe structurally can't verify: (1) VoiceOver/NVDA heading navigation reaches and announces each step's `<h1>` after every transition; (2) `SaveIndicator`'s save-state announcements land without interrupting typing; (3) `ProductErrorPanel`'s `role="alert"` interrupts appropriately on a failed save/422; (4) `OperationTable`'s row navigation is announced sensibly (not silent) and the checkbox's name+state read correctly; (5) `RiskAcknowledgeDialog` traps focus on open and returns it to the triggering control on close; (6) the "Download .zip" link's purpose is clear from its text alone, out of context | P4 | P3-W19-E01 | M | 4–6 | NFR-ACCESS, BRD §35 |
...
| `GEN-006` | GENERATION | Generated package is {bytes} bytes, exceeding the {cap}-byte cap | Reduce the tool surface or split into multiple projects |
...
| `PLG-001` | SECURITY | This tool is Destructive or Privileged and requires explicit acknowledgement before it runs | Acknowledge the risk in the playground to proceed |
| `PLG-002` | SECURITY | Blocked live-execute egress to a private or disallowed target | Use a publicly resolvable HTTPS target, or set `MCPGEN_ALLOW_PRIVATE_EGRESS` for local development |
...
| **C26** | Increment 9 pulled Playwright forward from Increment 12 (real-browser verification for every UI increment from here on, not just curl-against-the-API) — the very first real-browser run of the wizard (Increments 3–8's pages, never actually opened in a browser before) surfaced two accessibility bugs that had been shipping silently since Increment 3: (1) `StepFooter`'s Back/Continue controls used Base UI's `Button` with a `render={<Link/>}` prop; Base UI's `Button` always exposes `role="button"` to the accessibility tree regardless of the underlying element (confirmed empirically, not assumed) — wrong for a control that navigates, since WAI-ARIA distinguishes links (navigate, support "open in new tab") from buttons (act in place). (2) `CardTitle` rendered a `<div>`, not a heading — every step's card-level heading was invisible to screen-reader heading navigation. | (1) Fixed by rendering real `<Link>` elements styled with `buttonVariants` instead of routing navigation through `Button`+`render` — `Button` now stays reserved for controls that actually perform an in-place action. (2) `CardTitle` now renders `<h2>` (the page's own `<h1>` from `StepShell` is the only other heading level in play, so `h2` is correct without renumbering anything). Both fixes are shared components, so they corrected every step at once rather than needing a per-page patch. New `apps/web/e2e/wizard-happy-path.spec.ts` (Playwright, real browser) now exercises import through a dry-run preview end to end and would catch a regression of either. |
| **C27** | Same real-browser run, a third bug: every `<Select>` across the wizard (auth type, binding kind, risk override, retry choice, tool pickers, filters — effectively every custom dropdown built since Increment 5) displayed the *raw* internal value in its trigger (`"READ_ONLY"`, `"oauth2ClientCredentials"`, `"environment"`) instead of its friendly label (`"Read only"`, `"OAuth2 (Client Credentials)"`, `"Environment variable"`). | Root cause: Base UI's `Select.Value` does not auto-resolve a label from the matching `Select.Item`'s children the way Radix's does — confirmed by reading Base UI's own `SelectValue.d.ts`, which documents a `children` render-prop (`(value) => ReactNode`) as the intended way to format the displayed value, with no `items`-map auto-lookup fallback for a bare `<SelectValue />`. Every one of the ~8 affected files (`ValueBindingField`, `SecretBindingField`, `RiskOverrideCell`, `RetryToggle`, `ToolFilters`, `AuthView` ×2, `BindingsView`, `PlaygroundView`) already had a `value → label` map sitting right next to the broken `<SelectValue />` (used correctly inside each `SelectItem`) — fixed by passing that same map through `<SelectValue>{(v) => LABELS[v]}</SelectValue>` at every call site. Purely visual (the underlying `value`/`onValueChange` wiring was always correct), but a real, user-facing regression that no vitest/jsdom test could have caught — jsdom never renders Base UI's popup positioning or reads the trigger's rendered text the way a real browser round-trip does. |
| **C28** | Two more real-browser findings, both silent gaps rather than incorrect behavior: (1) a failed autosave (`SAVE_FAILED`, e.g. a `422` from an incomplete OAuth2 form) updated `wizardState.errors` correctly, but no editable step ever rendered that state — a failed save was visible only as the word "Save failed" in `SaveIndicator`, with no explanation. (2) `ToolsView` showed "No operations match the current filters" during the brief window `useProjectQuery(['operations'])` was still loading, indistinguishable from a genuine zero-result state. | (1) New shared `SaveErrorBanner` (reads `saveStatus`/`errors` from wizard state, dismissible via `ERRORS_DISMISSED`) added to every editable step alongside `ConflictBanner`. (2) `ToolsView` now checks `operationsQuery.isLoading` before falling through to the empty-state copy. Both are the kind of "nothing is technically broken, but the user gets no signal" gap that only surfaces when you actually watch a page load in a browser rather than assert on route-handler JSON. |
| **C29** | While building and browser-verifying Increment 11 (Generate), extending `wizard-happy-path.spec.ts` through a real Generate click surfaced a significant pre-existing bug from Increment 2: `gates.generate.reachable` was permanently `false` for any realistic project using secret-based or unresolved-environment-based upstream authentication — i.e. almost every real API. `computeGateInput()` (`apps/web/src/server/snapshot.ts`) calls `validateStartupRequirements` with a deliberately stub `resolveSecret: async () => undefined`/no-default-env context (documented in its own comment as "structural-only, no real secrets at snapshot time" — correct for `apps/cli`'s `validate` command, which runs against a real deployment environment). But `binding-engine`'s `resolveOne()` (`packages/binding-engine/src/resolve-value.ts`) has no "declared but not yet resolvable" state for a `secret`-sourced binding — an unresolved secret always produces an error-severity `AUT-001` diagnostic, same for an `environment`-sourced binding with no default (`BND-005`). Since `generateReachable` was computed as `configParses && allEnabledToolsResolveToOperations && startupValid`, any secret anywhere in the config (base URL, auth, or any enabled tool's bindings) made `startupValid`, and therefore the Generate gate, unconditionally `false` — confirmed via two direct curl checks against the live server: a fresh project with default seeded bearer auth (blocked), then the same project with a tool enabled and the base URL switched to a fully static, resolvable value, leaving *only* the bearer secret unresolved (still blocked) — isolating the blocker to the secret specifically, not the base URL. | `packages/control-contracts/src/gates.ts`: `generateReachable` now depends on `allEnabledToolsHaveRequiredBindings` instead of `startupValid` — binding *presence* (every required field has some binding declared) is the correct design-time gate; binding *value resolvability* is legitimately a deploy-time concern this product's ephemeral, no-accounts architecture can't and shouldn't require up front. `startupValid` stays in `GateInput` (still computed, still an honest signal of deploy-readiness) but is no longer wired to any gate — the interface doc comment explains why, so a future reader isn't tempted to "fix" the apparent dead field by re-wiring it. Re-verified live: the same two-curl reproduction now shows `generate.reachable: true` at both steps, and a real `POST .../generate` against the secret-auth project succeeds (7 files, real `dist/cli.mjs`). `gates.test.ts` gained a dedicated regression case asserting `generate.reachable` stays `true` when `startupValid` is `false` but bindings are complete. Found by the same "actually click Generate in a real browser" discipline C26–C28 already established — no unit or integration test had ever exercised the gate with a realistic (secret-auth) seeded project, since every prior fixture either had zero enabled tools or used `MCPGEN_ALLOW_PRIVATE_EGRESS`-style test doubles that supplied a real secret up front. |
| **C30** | Fixing C29 changed `gates.generate.complete` from always-`false` to genuinely `true` once every enabled tool's required parameters are bound (the realistic case for a seeded project with default tool-input bindings) — which flipped `StepNav`'s sidebar "Generate" link to render an accessible name of `"Generate (complete)"` (a screen-reader-only suffix added in Increment 3/C26's accessibility work). That collided with `PlaygroundView`'s own `StepFooter` continue button, whose `continueLabel` was the bare string `"Generate"` (`en.stepGenerate`) — the one step in the wizard that broke the otherwise-consistent `"Continue to <next step>"` naming every other `StepFooter` continue button uses. Two links with overlapping accessible names on the same page is a real WCAG 2.4.4-adjacent naming smell, not just a Playwright selector inconvenience — it surfaced immediately once the extended `wizard-happy-path.spec.ts` tried to click "Generate" and hit Playwright's strict-mode ambiguity error against two real matches. | Added `en.playgroundContinue = 'Continue to generate'`, matching the established `<step>Continue` naming convention, and pointed `PlaygroundView`'s `StepFooter` at it instead of the bare `stepGenerate` string. The e2e spec now targets `"Continue to generate"` unambiguously. Re-ran the full Playwright suite under its normal `fullyParallel: true`/3-worker config to also check the `playground-live-execute.spec.ts` timeout flake noted after Increment 10 — both of its tests passed cleanly alongside the extended happy-path spec, supporting the resource-contention hypothesis (a stale dev-server process left over from manual verification) over a real product or test bug, though this was only re-run once and isn't a guarantee against a rarer race. |
| **C31** | Increment 12's two new systematic specs (`a11y.spec.ts`, `keyboard.spec.ts`) each found one genuine bug the earlier, narrower real-browser passes (C26–C30) hadn't exercised: (1) `@axe-core/playwright` against all 10 routes flagged `color-contrast` (`serious`) on both `Button`'s and `Badge`'s `destructive` variant — `text-destructive` (`#e7000b`) on `bg-destructive/10` (`#fde6e7`) is a 4.0:1 contrast ratio, below WCAG 2 AA's 4.5:1 minimum for normal text; every destructive-styled control in the wizard (binding-row Remove buttons, required-field badges, etc.) inherited it from the shared `buttonVariants`/`badgeVariants`. (2) The keyboard-only traversal test tried to Tab to `OperationTable`'s enable checkbox and toggle it with Space, and instead the tool designer opened — the checkbox's `Space` keydown bubbles (React's synthetic event system mirrors native bubbling) up into the row's own roving-tabindex `onKeyDown` handler, whose `case ' '` branch calls `event.preventDefault()` and `onSelect(...)`; the checkbox's existing `onClick`'s `event.stopPropagation()` had no `onKeyDown` counterpart, so a keyboard user could never actually toggle the checkbox once focus was on it (a mouse click worked fine, since `onClick` was already guarded — this was keyboard-only). | (1) Both variants' text color changed to `text-red-700`/`dark:text-red-400` (same hue as the `--destructive` token, chosen specifically to clear 4.5:1 against their existing backgrounds) — one shared-component fix, verified by axe going green across all 10 routes. (2) Added `onKeyDown={(event) => event.stopPropagation()}` to the checkbox, mirroring its existing `onClick` guard. Neither bug was reachable by `wizard-happy-path.spec.ts` (which only ever clicks) or by the unit/component suites (jsdom doesn't compute real contrast, and no test had exercised the checkbox via a real keydown) — both are exactly the class of finding this increment's systematic, every-route/every-widget coverage exists to catch before it ships. |
| **C32** | A post-Increment-12 design audit (user-requested, not tied to a WBS row) walked all 10 wizard routes via real-browser screenshots against the actual code and found the shipped UI was the unmodified shadcn "neutral" theme — every color token zero-chroma (pure black/white/gray) except `--destructive` — with no persistent app header/identity past the import step, and required-vs-optional field state genuinely undiscoverable outside the Bindings table (Tools designer's Name/Description, Auth's credential fields, API Defaults' Base URL, and every Playground input field carried no indicator at all). Also found by direct screenshot comparison: opening the Tools step's designer panel visibly smeared the operation table's row backgrounds — `ToolsView.tsx`'s `grid-cols-[1fr_360px]` container has no `items-start`, so the (shorter) `<table>` stretches to match the (taller) designer panel via CSS Grid's default `align-items: stretch`, and the browser's table layout algorithm redistributes that extra height into the rows. Safety step's two behavioral caveats (no confirmation-before-execution, destructive/privileged retry floor) rendered as plain neutral cards indistinguishable from filler text, despite the rest of the app consistently using icon+color for anything advisory. | Added real `--primary` (blue, ~Tailwind blue-600/blue-400 dark) and `--success` (green, ~emerald-600/emerald-400 dark) tokens to `globals.css`, replacing the achromatic default — every `bg-primary`/`text-primary` consumer (default-variant buttons, links, focus rings) inherited the new blue for free; `ScoreDial`'s already-correct semantic banding (`bandFor`) now resolves "Strong" to real green instead of near-black, and `SaveIndicator`'s "Saved" state matches. New `AppHeader` (wordmark + link home) in the root layout, and `StepShell` now shows the open project's name above `StepNav` (both read from data that already existed — `ProjectSnapshot.name` — just not surfaced anywhere). `StepNav` upgraded from a `sr-only`-only "(complete)" suffix to a visible green checkmark per completed step plus a blue-tinted current-step highlight — still WCAG non-color-alone since the checkmark shape and blue text both carry the signal independent of color, and every route re-passed axe with zero serious/critical violations after the change. Added `required`/`optional` props directly to the shared `Label` primitive (asterisk + sr-only "required" text, or a muted "(optional)" suffix) and threaded them through `ValueBindingField`/`SecretBindingField` to every genuinely-required field across API Defaults, Auth (all 4 variants), and the Tools designer's Name field (Description marked optional). Playground's `ToolInputForm` needed real plumbing — it only had `ToolConfig`, not the operation's actual parameter schema — so `PlaygroundView` now also fetches `operationDetail` for the selected tool and passes it through `PlaygroundToolPanel`; a first pass at this had a real bug (caught by a manual verification screenshot, not a test): while `operationDetail` was still loading, every field showed "(optional)" including the genuinely-required `customer_id`, since the required-lookup returned `false` for an empty set rather than "unknown" — fixed by making `required` a three-state `true \| false \| undefined` (unmarked while loading) instead of a boolean. `ToolsView.tsx`'s grid gained `items-start`, verified by screenshot (no more row-background smear). `Alert` gained a `warning` variant (amber tint + `Info` icon) applied to both Safety-step caveats. Explicitly not purple, not a generic "AI gradient" look — blue/green/amber/red is the same professional palette already specified for this project's own Mermaid diagram conventions. Re-verified: full build/typecheck/lint/unit/integration/security green, and the complete Playwright suite (a11y × 10 routes, keyboard, live-execute, happy-path) green against the new palette — axe would have caught a contrast regression from the new colors and didn't. |
| **C33** | Building `apps/web/Dockerfile` (a one-command local demo, previously speced in the web-UI plan's Deployment section but never actually built) surfaced five distinct, genuine bugs — none hypothetical, every one reproduced and root-caused before being fixed: (1) `turbo run build`'s default concurrency let a dependent package's `tsc` start before its dependency's (`@mcpgen/domain`) `tsc` had finished flushing declaration files — reproduced both inside the container and on the host once `.turbo`/`dist`/`tsconfig.tsbuildinfo` were fully cleared, confirmed to be a real execution race rather than a `turbo.json`/`dependsOn` graph error (`turbo run build --dry=json` showed the dependency edge was correctly declared). (2) The original `.dockerignore` used `*.tsbuildinfo` (unanchored) instead of `**/*.tsbuildinfo`, so the pruner stage's `COPY . .` picked up leftover incremental-build metadata from the *host* machine — TypeScript's composite/incremental mode trusts that file over on-disk output, so it silently skipped emitting anything it believed was already built, producing a container that looked freshly built but wasn't. (3) `output: 'standalone'` (Next's own dependency-tracing mode, tried first as the standard approach) missed `@swc/helpers`'s `esm/` files — a documented Next.js + pnpm gap — and, more fundamentally, couldn't trace `@mcpgen/generator`'s `bundleRuntime` step at all, since it reads runtime-template files from `packages/generator/dist/runtime-entry/` by a computed path at request time, not via a statically-analyzable `require()`. Rather than patch tracer gaps one at a time as they're found, the Dockerfile ships the full built monorepo (via `turbo prune` for the install-cache layer, `docker-compose.yml`) instead of a traced subset — a real image-size-for-correctness trade, made explicitly rather than discovered as a surprise in production. (4) The non-root runtime user had no home directory, so `corepack`'s pnpm-version verification (triggered by the original `CMD ["pnpm", "--filter", "@mcpgen/web", "start"]`) couldn't create its cache dir. (5) Even after fixing (4), pnpm 11's own pre-script dependency-status check failed under the non-root user for a separate permissions reason — worked around by having `CMD` invoke `apps/web/node_modules/.bin/next start` directly, skipping pnpm/corepack at container *runtime* entirely (they're only needed at *build* time). | Fixes, in order: (1)/(2) `--concurrency=1` on the Dockerfile's `turbo run build` invocation plus `.dockerignore`'s pattern corrected to `**/*.tsbuildinfo`. (3) Reverted the `output: 'standalone'` experiment in `next.config.ts`; `apps/web/Dockerfile` now installs and builds the full `turbo prune`-pruned monorepo and ships it as-is. (4) `useradd --create-home`, `chown` the new home directory, `ENV HOME=/home/mcpgen`. (5) `CMD` changed to the direct binary path. Verified genuinely end to end, not just "container starts": `docker compose up`, then a real HTTP client script drove import → enable a tool → set a static base URL → `PUT /config` → `POST /generate` → `GET /generate/:buildId/download` entirely against the running container, followed by extracting the downloaded zip, `npm install`, and running the generated server for real (`node dist/cli.mjs`) *outside* the container — confirmed it produces the expected `AUT-001` startup diagnostic for the intentionally-unset secret, the same behavior verified via the CLI and the dev server throughout this session. `README.md`'s Docker section documents the `WEB_PORT` override discovered necessary on this exact machine (port 3000 already held by an unrelated local service). |
| **C34** | The onboarding/distribution follow-up (user-reported after testing on a second machine: nothing told a new user how to run any of this) covered six phases — CLI `bin`, README Quickstart, root convenience scripts, Docker Compose (C33), a publishable landing page plus `/docs`, and honest optional-step affordances in the wizard. Three findings are worth recording beyond C33's Docker work. (1) A genuine pre-existing **packaging** bug, not a doc gap: `apps/cli/package.json` had no `bin` field at all, so `dist/cli.js`'s `#!/usr/bin/env node` shebang was dead weight and no amount of `npm link` could produce a working `mcpgen` command — the CLI was documented as installable and structurally was not. (2) The wizard's "optional" steps were already skippable (`StepFooter` only ever disables Continue on blocking validation errors or zero enabled tools), so the gap was purely communicative — but the naive fix, a flat `optional: boolean` per step, would have made a claim the product cannot honor: the import seeds `upstreamAuthentication` from the spec's primary security scheme, so marking Authentication unconditionally optional would tell a user with a credentialed API that they may skip configuring credentials, producing a server that starts and then fails every upstream call. (3) The new public pages surfaced two real accessibility defects the existing suite caught immediately once extended: `text-muted-foreground` (#737373) on `bg-muted` (#f5f5f5) is 4.34:1, under WCAG AA's 4.5:1 — a token pairing the palette established in C32 never intended and no prior screen had used — and `/docs`'s horizontally-scrolling `<pre>` snippets were unfocusable at 375px, so a keyboard user could not scroll them to read the rest of the command (axe `scrollable-region-focusable`). | Fixes: (1) `bin: { mcpgen: './dist/cli.js' }`, mirroring the pattern `packages/generator/src/package-json.ts` already emits for *generated* packages, verified by a real `npm link` and an end-to-end run against the repo's own fixture. (2) `optional` is the *baseline* claim only; `isStepOptional(stepId, { hasUpstreamAuth })` resolves it, and Authentication resolves to skippable only when the document declared no scheme. `StepNav` tags it, `StepShell` explains it, `StepFooter` gained a lower-emphasis "Skip for now" link. `apps/web/e2e/optional-steps.spec.ts` pins both directions in a real browser — skipping Safety and Test still reaches Generate, and `/auth` offers no skip affordance for the `bearerAuth`-declaring fixture. `/docs` repeats the same discipline: with no project open it cannot resolve the auth branch, so it renders a qualified "Optional when your spec declares no security scheme" rather than the flat marker. (3) The muted-on-muted surfaces moved to Card's established `bg-card` + `ring-1 ring-foreground/10` treatment instead of inventing a panel the palette does not support; `CodeBlock` gained `tabIndex={0}` with `role="region"` and an accessible name. That second fix collided head-on with `jsx-a11y/no-noninteractive-tabindex`, whose default allowlist predates the axe rule and permits only `tabpanel` — the two lint each other's fixes, resolved by adding `region` to the rule's `roles` option (repo-wide, in the `apps/web` block) rather than a local disable comment, since the axe check is the one grounded in real assistive-technology behavior and `region` still requires an accessible name. Landing-page verification was a real visual round-trip, not a build-and-ship: `/` and `/docs` screenshotted at 375/768/1440 and read as a first-time visitor would, with axe extended to cover both routes at desktop *and* 375px (the mobile layout is genuinely different markup, not the same DOM reflowed). The hero is a real screenshot of the running wizard captured by `apps/web/scripts/capture-hero.mjs` — annotations are deliberately captions *outside* the image rather than overlays drawn on top, since overlay coordinates positioned against a specific PNG drift silently the moment it is regenerated. Both pages take their step labels from `WIZARD_STEPS` so a step rename cannot leave the marketing copy describing an older product. One further gotcha, found by driving the *production* build rather than the dev server: `/` and `/docs` are statically prerendered, so `metadataBase` — and with it the absolute Open Graph card URL — is fixed when `next build` runs. Setting the new `MCPGEN_PUBLIC_URL` on a running container is too late and silently leaves the card pointing at the `http://localhost:3000` default, which looks correct in the container's own env listing. Reproduced (runtime value ignored) and fixed by making it a Dockerfile `ARG` plumbed through `docker-compose.yml`'s `build.args`. That fix then failed its own verification in a way worth recording: building the image with a real origin still produced a `localhost:3000` card URL, because **turbo runs tasks with a filtered environment** — an undeclared variable is dropped before `next build` ever sees it, and is also absent from the task's cache key, so a stale output would be reused across different values. The first round of verification missed this only because it invoked `next build` directly on the host, bypassing turbo entirely; the container, which builds through `turbo run build`, is what exposed it. Resolved by declaring `env: ["MCPGEN_PUBLIC_URL"]` on turbo.json's `build` task, and confirmed by reading the served `og:image` out of the running container rather than out of a host build. The general lesson: a build-time variable in this repo needs three things, not one — a Dockerfile `ARG`, a compose `build.args` entry, and a turbo `env` declaration; missing the third fails silently. |
