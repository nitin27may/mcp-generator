...
| P2-W23-E01 | **In progress (4/11 routes).** Next.js Route Handlers per §53 (not Fastify — §93 C21); job pattern deferred, not yet needed. Done: `POST /api/import`, `POST /api/projects` (seeds a draft `McpProjectConfig` via `seedProjectConfig`), `GET /api/projects/:id` (with `computeGates`, now supports `?include=analysis`), `POST /api/projects/:id/analyze` (`src/server/analyze.ts`, skip-recompute via `sourceFingerprint`). `packages/control-contracts` (shared schemas + `computeGates`) and the disk-backed project store (`apps/web/src/server/project-store.ts`, TTL-swept via `instrumentation.ts`) landed alongside; store gained `readProjectAnalysis`/`writeProjectAnalysis`. Remaining: `config`, `playground/*`, `generate`, `source-versions`, `diff` | P2 | P1-W26-T01 | L | 6–10 | §53 |
